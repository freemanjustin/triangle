...
const double B_ERR_MIN_SQ = ((float)(DBL_EPSILON*MAXDIM*(1<<MAXDIM)*MAXDIM*3.01)) * ((float)(DBL_EPSILON*MAXDIM*(1<<MAXDIM)*MAXDIM*3.01));
...
static void build_convex_hull(void) {
   ...
   rdim = pdim+1;
   if (rdim > MAXDIM){
      fprintf(stderr,"dimension bound MAXDIM exceeded; MAXDIM = %d, rdim=%d; pdim=%d\n", MAXDIM, rdim, pdim);
